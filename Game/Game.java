 ...
 public static void main(String[] args) {
  ...
  for (int a = 0; a < guessnum; a++) {
   ...
   try {
    ...
    userline = userguess.readLine();
    userlinechar = userline.toCharArray();
    finalGuess = userlinechar[0];

    for (int x = 0; x < chosenarr.length; x++) {
     for (int j = 0; j < output.length; j++) {

	if (Character.toLowerCase(finalGuess) == Character.toLowerCase(chosenarr[x])) { 
       output[x] = Character.toLowerCase(finalGuess);
      }
     }
